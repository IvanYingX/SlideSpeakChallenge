...
async def get_system_stats():
    ...
    completed = sum(1 for d in document_store.values() if d.get("status") == "complete")
    errored = sum(1 for d in document_store.values() if d.get("status") == "error")
    avg_time = 0.0
    durations = []

    for doc in document_store.values():
        result = doc.get("result")
        if result and result.get("processing_time_seconds"):
